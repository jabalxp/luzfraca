// ============================================================
// LUZ FRACA — map.js : geração procedural dos 10 mapas
...
const TILE = { FLOOR: 0, WALL: 1, WALL2: 2, EXIT: 3, HIDE: 4, WATER: 5, DARK: 6, PETAL: 7, GOD: 8, GRASS: 9 };
...
function generateMap(level, nightmare) {
...
function genArena(m, rect, ri, rng) {
  ...
  m.paper = { x: 23.5, y: 23.5 };                      // marca do pênalti, centro do gramado
  m.exit = { x: 23, y: 6 };                            // portão atrás do gol norte
  m.set(23, 6, TILE.EXIT);
  m.patrolHint = 'ring';                               // 12º Jogador: linha lateral
  m.props.push({ x: 23.5, y: 13.8, type: 'goal' }, { x: 23.5, y: 33.8, type: 'goal' }, { x: 10, y: 10, type: 'scoreboard' });
}

// --- FASE 2: walmart — grade de prateleiras 8x6 ---
function genShelves(m, rect, ri, rng) {
  rect(3, 3, 44, 44, TILE.FLOOR);
  // prateleiras: paredes em fileiras
  ...
  // estoque dos fundos (norte)
  rect(3, 3, 44, 5, TILE.FLOOR);
  for (let x = 8; x < 42; x += 6) { m.set(x, 4, TILE.WALL2); }
  // sala do gerente (canto do estoque)
  rect(38, 2, 44, 6, TILE.FLOOR); rect(37, 2, 37, 6, TILE.WALL); m.set(37, 4, TILE.FLOOR);
  m.paper = { x: 41.5, y: 3.5 };
  // caixas (sul) e doca
  rect(5, 40, 42, 44, TILE.FLOOR);
  for (let x = 8; x < 40; x += 5) m.set(x, 41, TILE.WALL2);
  m.spawn = { x: 23.5, y: 42.5 };
  m.exit = { x: 5, y: 3 }; m.set(5, 3, TILE.EXIT);     // doca de carga no estoque
  m.props.push({ x: 12.5, y: 22.5, type: 'cart' }, { x: 30.5, y: 17.5, type: 'cart' }, { x: 20.5, y: 32.5, type: 'cart' });
}

// --- FASE 3: backrooms — labirinto amarelo + 3 âncoras ---
function genMaze(m, rect, ri, rng) {
  // labirinto por divisão de células 2x2
  for (let y = 1; y < MAP_H - 1; y++) for (let x = 1; x < MAP_W - 1; x++) {
    if (x % 2 === 1 || y % 2 === 1) m.set(x, y, TILE.FLOOR);
  }
  // re-adiciona paredes para formar corredores
  ...
  // âncoras: sala das pilastras, sala alagada, sala escura
  rect(6, 6, 14, 14, TILE.FLOOR);
  for (let y = 8; y <= 12; y += 2) for (let x = 8; x <= 12; x += 2) m.set(x, y, TILE.WALL2);
  rect(32, 30, 42, 40, TILE.WATER);                    // sala alagada (carpete encharcado)
  ...
  m.exit = { x: 45, y: 24 }; m.set(45, 24, TILE.EXIT); // buraco na parede ("no-clip")
...
function genVillage(m, rect, ri, rng, level) {
  ...
  if (level.candles) {           // México: velas + caminhos de pétalas
    for (let i = 0; i < 16; i++) m.props.push({ x: ri(4, 43) + 0.5, y: ri(13, 43) + 0.5, type: 'candle' });
    let px = 24, py = 42;        // caminho de pétalas serpenteando ao norte
  ...
  if (level.offerings) {         // Paraguai: 7 cruzes + 3 garrafas de caña
    ...
    m.paper = { x: 36.5, y: 37.5 }; // pregado na sétima cruz
...
function genSavanna(m, rect, ri, rng) {
  rect(2, 2, 45, 45, TILE.FLOOR);
  // manchas de capim alto
  for (let i = 0; i < 14; i++) {
    const cx = ri(5, 42), cy = ri(5, 42), r = ri(2, 5);
    for (let y = cy - r; y <= cy + r; y++) for (let x = cx - r; x <= cx + r; x++) {
      if ((x - cx) ** 2 + (y - cy) ** 2 <= r * r && m.at && m.t[y * MAP_W + x] === TILE.FLOOR) m.set(x, y, TILE.GRASS);
    }
  }
  // cabanas circulares
  for (let i = 0; i < 6; i++) {
    ...
    m.rooms.push({ cx, cy, building: true });
  }
  // baobás
  for (let i = 0; i < 5; i++) m.props.push({ x: ri(4, 43) + 0.5, y: ri(4, 43) + 0.5, type: 'baobab', solid: true });
  // poço central — o papel está NO FUNDO (interação demorada)
...
function genFacility(m, rect, ri, rng) {
  ...
  // quarentena selada com "plástico" (passável mas marcada)
  m.paper = { x: 39.5, y: 6.5 };
  m.props.push({ x: 39.5, y: 8.5, type: 'gurney' }, { x: 22.5, y: 6.5, type: 'console' }, { x: 8.5, y: 7.5, type: 'rocket', solid: true });
...
function genTemple(m, rect, ri, rng) {
  ...
  for (let x = 17; x < 31; x += 3) { m.set(x, 6, TILE.WALL2); m.set(x, 9, TILE.WALL2); } // bancos
  ...
  rect(14, 12, 17, 20, TILE.FLOOR); rect(30, 12, 33, 20, TILE.FLOOR); // ligações
  rect(4, 12, 7, 16, TILE.FLOOR); rect(40, 12, 43, 16, TILE.FLOOR);
  rect(4, 16, 43, 20, TILE.FLOOR);                     // claustro
  rect(10, 20, 13, 30, TILE.FLOOR); rect(34, 20, 37, 30, TILE.FLOOR); // túneis cavados à mão
  rect(22, 20, 25, 30, TILE.FLOOR);
  rect(8, 30, 39, 33, TILE.FLOOR);                     // antecâmara
  // Salão da Voz (o subsolo do Deus Cego)
  ...
  m.paper = { x: 23.5, y: 43.5 };                      // sobre a língua de pedra do ídolo
  ...
  m.exit = { x: 23, y: 3 }; m.set(23, 3, TILE.EXIT);   // sai por onde entrou
}

// ---------- conectividade ----------
function ensureConnectivity(m) {
  ...
  // cavar túneis até POIs inalcançáveis
  const carveTo = (tx, ty) => {
    let x = sx, y = sy;
    while (x !== tx || y !== ty) {
      if (x !== tx) x += Math.sign(tx - x); else y += Math.sign(ty - y);
      if (!pass(m.t[idx(x, y)]) || m.t[idx(x, y)] === TILE.WALL || m.t[idx(x, y)] === TILE.WALL2) {
...
function placeEntities(m, level, nightmare, ri, rng) {
  ...
  freeTiles.sort((a, b) => pd(b) - pd(a)); // longe do papel primeiro

  // moedas: 60% rotas seguras (longe do papel), 30% médias, 10% zona de morte (x2)
  const n = level.coins;
  ...
  // esconderijos: tiles livres adjacentes a paredes
  let placed = 0;
  ...
  // papel em novo esconderijo no Modo Pesadelo
...
function isSolidTile(v, exitOpen) {
...
function bfsNextStep(m, sx, sy, tx, ty, exitOpen) {
  const idx = (x, y) => y * MAP_W + x;
...
function lineOfSight(m, x0, y0, x1, y1) {
