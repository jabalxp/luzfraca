...
// Reverb por convolução dá profundidade de "lugar grande e vazio".
// ============================================================
import { SAVE } from './save.js';

export const AudioSys = {
  ctx: null, master: null, music: null, fx: null, scare: null,
  droneNodes: [], chaseNodes: [], started: false,
  noiseBuf: null, reverb: null,
  ...
  init() {
    ...
    // reverb de impulso sintético (espaço grande, decaimento 2.4s)
    const rlen = this.ctx.sampleRate * 2.4;
    const ir = this.ctx.createBuffer(2, rlen, this.ctx.sampleRate);
    for (let c = 0; c < 2; c++) {
      const ch = ir.getChannelData(c);
      for (let i = 0; i < rlen; i++) ch[i] = (Math.random() * 2 - 1) * Math.pow(1 - i / rlen, 2.6);
    }
    ...
    this.scare.connect(this.reverb);
    this.started = true;
    this.applyVolumes();
  },
  ...
  startDrone(kind) {
    ...
    switch (kind) {
      ...
      case 'crickets': this.cricketGain = mkNoise(4200, 8, 0.025); mk('sine', 55, 0.04); break;
  ...
  stopDrone() {
    for (const n of this.droneNodes) { try { n.stop ? n.stop() : n.disconnect(); } catch { /* já parado */ } }
    this.droneNodes = []; this.humOsc = null; this.cricketGain = null;
  },
  setHumProximity(p) { // Errante: zumbido muda de tom quando perto (0 longe → 1 colado)
  ...
  stopChase() {
    for (const n of this.chaseNodes) { try { n.stop ? n.stop() : n.disconnect(); } catch { /* já parado */ } }
    this.chaseNodes = [];
  },
